  ...
  */
  constructor(defaultHandler = () => {}) {
    if (window.routerInstance === undefined) {
      ...
      let setLocation = e => {
        let hash = (e.newURL || document.location.hash)
          .slice((e.newURL || document.location.hash).indexOf('#')),
          matches = 0;
  ...
  extendRoute(route, handler){
    let fn = this.lookup[route];
    this.lookup[route] = () => {
      fn();
  ...
  */
  addRoutes(...routes) {
    routes.forEach(routeObj => {
      ...
      this.matchers.push(
        new RegExp(routeObj.route.replace(/:[^\s/]+/g, '([\\w-]+)'))
      );
